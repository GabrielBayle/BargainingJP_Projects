...
#    - DONE:  replace instruction images with 2 options only 
#    - DONE: make sure instructions are coherent with the within treatment and only 2 states 
#    - DONE: 1/2 informed at random: make sure it is the young with the exception of the first round   
#    - DONE: just one dividend for the whole thing 
#    - DONE: adjust instructions accordingly4

#    - DONE:  check history for is_informed
#    - DONE: check quiz
# 
#    - Done: Costly Information Treatment!!!!!! --> Extra screen for deciding whether to be informed or not!@  
#    - DoneL Costly Information Instructions,,,, check it makes sense

#     ---> Improve graph: only update if new transaction, not for every bid or ask! 

# NOT SO IMPORTANT:


#   -DONE:  show entering participants  past average_price (plus click on graph)    ask
#   -DONE: make bus/sell notice bigger  ?????????

#   - DONE: make sure tio record attempts in quiz    
#   - DOne:L timer on Sequence Results  20 seg
#   - Done: make sure it is production mode!!
...
class Group(BaseGroup):
    ...
    world_state = models.StringField()   # random the world
        ...
        if p.is_playing is True:    # players  payoff
            ...
            if (p.round_number == p.session.config['dividend_1_period']) or (p.round_number == p.session.config['dividend_2_period']) or \
               (p.round_number == p.session.config['dividend_3_period']) or (p.round_number == p.session.config['dividend_4_period']) or  \
                ...
                (p.round_number == p.session.config['dividend_7_period']) or (p.round_number == p.session.config['dividend_8_period']) :                 
...
def sequence_payoff_olg(group: Group):
  ...
  #              if p.round_number == p.session.config['dividend_10_period']:
       ...
       #             p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
        #                , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) , float(p.session.config['dividend_7_period'])\
         #               , float(p.session.config['dividend_8_period']) , float(p.session.config['dividend_9_period'])], 1)
                if p.round_number == float(p.session.config['dividend_8_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) , float(p.session.config['dividend_7_period'])\
...
def new_sequence(group: Group):
    ...
    count_aux_old=1
    for p in players:
        p.is_playing = p.id_in_group <= p.session.config['num_old']+p.session.config['num_young']    # the number of participants playing
        p.is_young= p.id_in_group > p.session.config['num_old']                                       # the number young                
    
        if p.is_playing:
            if  p.is_young is True:    # values for the first cohort's young 
                p.is_buyer = True
                p.num_items = 0
                ...
                p.is_young=False      # values for the first cohort's old 
                p.is_buyer = False
...
def dividends_titles_and_clues(group: Group):
    ...
    # dividend per sequence and clues 
        ...
        if player.round_number <= player.session.config['dividend_8_period']:
            ...
            player.dividend_clue = player.session.config['dividend_8']  
            player.dividend_clue_text=player.session.config['state_8']  
        ...
        if player.round_number <= player.session.config['dividend_7_period']:
            ...
            player.dividend_clue = player.session.config['dividend_7']  
            player.dividend_clue_text=player.session.config['state_7']   
        ...
        if player.round_number <= player.session.config['dividend_6_period']:
            ...
            player.dividend_clue = player.session.config['dividend_6']  
            player.dividend_clue_text=player.session.config['state_6']   
        ...
        if player.round_number <= player.session.config['dividend_5_period']:
            ...
            player.dividend_clue = player.session.config['dividend_5']  
            player.dividend_clue_text=player.session.config['state_5']  
        ...
        if player.round_number <= player.session.config['dividend_4_period']:
            ...
            player.dividend_clue = player.session.config['dividend_4']  
            player.dividend_clue_text=player.session.config['state_4'] 
        ...
        if player.round_number <= player.session.config['dividend_3_period']:
            ...
            player.dividend_clue = player.session.config['dividend_3']  
            player.dividend_clue_text=player.session.config['state_3']   
        ...
        if player.round_number <= player.session.config['dividend_2_period']:
            ...
            player.dividend_clue = player.session.config['dividend_2']  
            player.dividend_clue_text=player.session.config['state_2'] 
        ...
        if player.round_number <= player.session.config['dividend_1_period']:
            ...
            player.dividend_clue = player.session.config['dividend_1']  
            player.dividend_clue_text=player.session.config['state_1'] 
    ...
    (group.round_number ==group.session.config['dividend_7_period']) or (group.round_number ==group.session.config['dividend_8_period']):
    ...
    (group.round_number == group.session.config['dividend_7_period']-1) or (group.round_number == group.session.config['dividend_8_period']-1):
    ...
    (group.round_number == group.session.config['dividend_7_period']-2) or (group.round_number == group.session.config['dividend_8_period']-2):
...
class PayingIntro(Page):
    ...
    def vars_for_template(player: Player):
        return dict(
            ...
            num_sequences_half=int(player.session.config['num_rounds']/6),
...
class InfoAcquisition(Page):
    timeout_seconds = 30
    
    @staticmethod
    def is_displayed(player: Player):
        return player.session.config['info_acquisition'] is True and player.is_playing is True and player.round_number <= player.session.config['num_rounds'] and (player.is_young is True or (player.round_number == 1) or \
        (player.round_number == player.session.config['dividend_1_period']+1) or (player.round_number == player.session.config['dividend_2_period']+1) or \
        (player.round_number == player.session.config['dividend_3_period']+1) or (player.round_number == player.session.config['dividend_4_period']+1) or \
        (player.round_number == player.session.config['dividend_5_period']+1) or (player.round_number == player.session.config['dividend_6_period']+1) or \
        (player.round_number == player.session.config['dividend_7_period']+1) or (player.round_number == player.session.config['dividend_8_period']+1) ) 
...
class Prediction(Page):
    @staticmethod
    def is_displayed(player: Player):
        return player.round_number <= player.session.config['num_rounds'] and player.is_playing is False
...
class SequenceResults(Page):
    ...
    (player.round_number == player.session.config['dividend_7_period']) or (player.round_number ==player.session.config['dividend_8_period']) ) and player.round_number<=player.session.config['num_rounds']
