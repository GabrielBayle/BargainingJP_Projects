...
class Player(BasePlayer):
    ...
    dividend_clue=models.FloatField()     # clue given to player
...
# assigns world state   
def assign_world_state(group: Group):
    players = group.get_players()
    group.world_state = numpy.random.choice(["A", "B", "C"])

# assign types 
def assign_types(group: Group):
    players = group.get_players()
    for p in players:    
        if (p.id_in_group == 1) or (p.id_in_group == 4) or (p.id_in_group == 7) or (p.id_in_group == 10) or (p.id_in_group == 13) or (p.id_in_group == 16):
            p.type  = "A"
        if (p.id_in_group == 2) or (p.id_in_group == 5) or (p.id_in_group == 8) or (p.id_in_group == 11) or (p.id_in_group == 14) or (p.id_in_group == 17):
            p.type  = "B"
        if (p.id_in_group == 3) or (p.id_in_group == 6) or (p.id_in_group == 9) or (p.id_in_group == 12) or (p.id_in_group == 15) or (p.id_in_group == 18): 
            p.type  = "C"

        #maintains the same type 
      #  else:
...
def round_payoff_olg(group: Group):
    ...
    for p in players:
        if p.is_playing is True:    # players  payoff
            ...
            if (p.round_number == p.session.config['dividend_1_period']) or (p.round_number == p.session.config['dividend_2_period']) or (p.round_number == p.session.config['dividend_3_period']) :                 
                if p.type == p.group.world_state:
                #    p.round_payoff =  float(p.session.config['dividend_1'])*float(p.num_items) + float(p.money)
                    p.round_payoff =  p.group.dividend*p.num_items + float(p.money)
                else:
    ...
    # normal rounds
                ...
                if p.is_young is False:                                                     # for old
                    p.round_payoff = float(p.money)        
            print('items=',p.num_items,'dividend=', p.session.config['dividend_1'], 'type=', p.type, 'world=', p.group.world_state , 'payoff=' ,p.round_payoff)
...
def sequence_payoff_olg(group: Group):
    ...
    for p in players:
            ...
            or p.round_number == p.session.config['dividend_3_period']:
                ...
                if p.session.config['info_acquisition'] is True and p.is_informed is True:
                    p.sequence_payoff=p.round_payoff+p.in_round(p.round_number-1).round_payoff+p.in_round(p.round_number-2).round_payoff-p.session.config['info_cost']
                else:
                    p.sequence_payoff=p.round_payoff+p.in_round(p.round_number-1).round_payoff+p.in_round(p.round_number-2).round_payoff
                print('CALCULATING SEQUENCE PAYMENT',  p.sequence_payoff )
...
def round_change_olg(group: Group):
    ...
    for p in players:
        ...
        # among those who did not play 
            ...
            # non-player who stays out because he is not selected
                else:
                    p.is_playing = False
                    p.num_items = 0
                    p.break_even_point = 0
                    ...
                    p.has_played = False
                    p.is_buyer = True
            # non-player who stays out because he already played before the previos round 
            else:
                p.is_playing = False
                p.num_items = 0
                p.break_even_point = 0
...
def dividends_titles_and_clues(group: Group):
    ...
    if (group.round_number == group.session.config['dividend_1_period']) or (group.round_number ==group.session.config['dividend_2_period']) or (group.round_number ==group.session.config['dividend_3_period']):
        group.title = 'Third and Final Round of the Sequence'
    elif (group.round_number == group.session.config['dividend_1_period']-1) or (group.round_number == group.session.config['dividend_2_period']-1) or (group.round_number == group.session.config['dividend_3_period']-1):
        group.title = 'Second Round of the Sequence'
    elif (group.round_number == group.session.config['dividend_1_period']-2) or (group.round_number == group.session.config['dividend_2_period']-2) or (group.round_number == group.session.config['dividend_3_period']-2):
        group.title = 'First Round of the Sequence'
...
class WaitToStart(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        if group.round_number == 1 or group.round_number == (group.session.config['dividend_1_period']+1) or group.round_number == (group.session.config['dividend_2_period']+1) \
        or group.round_number == (group.session.config['dividend_3_period']+1):
            new_sequence(group)
        else:
