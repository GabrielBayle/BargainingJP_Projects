...
def compute_payoff(player: Player):
    txt_final = f"ランダムに選ばれたのは表 {player.group.selected_choice} です。"
    if player.group.selected_player == player.id_in_group:
        ...
        txt_final += (f"報酬の計算にあなたの配分が選ばれました。自分に {player.group.selected_player_decision_self} ECU、"
                      f"もう一人のプレイヤーに {player.group.selected_player_decision_other} ECU を選択しました。したがって、あなたの報酬は "
                      f"{player.payoff} で、もう一人のプレイヤーの報酬は "
                      f"{cu(player.group.selected_player_decision_other * C.conversion_rate)} です。")
        player.decider = "あなたの決定"

    else:
        player.payoff = cu(player.group.selected_player_decision_other * C.conversion_rate)
        txt_final += (f"報酬の計算にもう一人のプレイヤーの配分が選ばれました。彼は自分に {player.group.selected_player_decision_self} ECU、"
                      f"あなたに {player.group.selected_player_decision_other} ECU を選択しました。したがって、あなたの報酬は {player.payoff} "
                      f"で、もう一人のプレイヤーの報酬は "
                      f"{cu(player.group.selected_player_decision_self * C.conversion_rate)} です。")
        player.decider = "もう一人の決定"
