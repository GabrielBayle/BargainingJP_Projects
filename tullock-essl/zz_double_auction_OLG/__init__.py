...
def round_payoff_olg(group: Group):
    ...
    for p in players:
        if p.is_playing is True:    # players  payoff
            ...
            if (p.round_number == p.session.config['dividend_1_period']) or (p.round_number == p.session.config['dividend_2_period']) or \
               (p.round_number == p.session.config['dividend_3_period']) or (p.round_number == p.session.config['dividend_4_period']) or  \
                (p.round_number == p.session.config['dividend_5_period']) or (p.round_number == p.session.config['dividend_6_period']) or \
                (p.round_number == p.session.config['dividend_7_period']) or (p.round_number == p.session.config['dividend_8_period']) or  \
                (p.round_number == p.session.config['dividend_9_period']) or (p.round_number == p.session.config['dividend_10_period']) :                 
...
def sequence_payoff_olg(group: Group):
    ...
    for p in players:
            ...
            or p.round_number == p.session.config['dividend_3_period'] \
            or p.round_number == p.session.config['dividend_4_period'] \
            or p.round_number == p.session.config['dividend_5_period'] \
            or p.round_number == p.session.config['dividend_6_period'] \
            or p.round_number == p.session.config['dividend_7_period'] \
            or p.round_number == p.session.config['dividend_8_period'] \
            or p.round_number == p.session.config['dividend_9_period'] \
            or p.round_number == p.session.config['dividend_10_period']:
                ...
                # USED WHEN PAYING ONly ONE Sequence AT RANDOM
            if p.round_number == p.session.config['num_rounds']:
                print('SELECTING ONE SEQUENCE AT RANDOM')
                if p.round_number == p.session.config['dividend_9_period']:
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) , float(p.session.config['dividend_7_period'])\
                        , float(p.session.config['dividend_8_period']) , float(p.session.config['dividend_9_period']), float(p.session.config['dividend_10_period'])], 1)
                elif p.round_number == p.session.config['dividend_9_period']:
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) , float(p.session.config['dividend_7_period'])\
                        , float(p.session.config['dividend_8_period']) , float(p.session.config['dividend_9_period'])], 1)
                elif p.round_number == float(p.session.config['dividend_8_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) , float(p.session.config['dividend_7_period'])\
                        , float(p.session.config['dividend_8_period'])], 1)
                elif p.round_number == float(p.session.config['dividend_7_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) , float(p.session.config['dividend_7_period'])], 1)
                elif p.round_number == float(p.session.config['dividend_6_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']), float(p.session.config['dividend_6_period']) ], 1)
                elif p.round_number == float(p.session.config['dividend_5_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) , float(p.session.config['dividend_5_period']) ], 1)
                elif p.round_number == float(p.session.config['dividend_4_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period'])\
                        , float(p.session.config['dividend_4_period']) ], 1)
                elif p.round_number == float(p.session.config['dividend_3_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) , float(p.session.config['dividend_3_period']) ], 1)
                elif p.round_number == float(p.session.config['dividend_2_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period']), float(p.session.config['dividend_2_period']) ], 1)
                elif p.round_number == float(p.session.config['dividend_1_period']):
                    p.participant.vars['paying_round'] = random.sample([float(p.session.config['dividend_1_period'])], 1)

                
                for y in p.participant.vars['paying_round']:
                    z = float(y)
                    print('selected sequence ends in round', z)
                    p.payoff += p.in_round(z).sequence_payoff / p.session.config['us_exchange_rate']
                    p.participant.vars['aux_payoff'] = p.in_round(z).sequence_payoff
                    print('paying round is', p.participant.vars['paying_round'], '=', p.participant.vars['aux_payoff'])
...
def dividends_titles_and_clues(group: Group):
    ...
    # clues for even and odd participants 
        if player.participant.id_in_session % 2 == 0:   # receive clue number one
            ...
            if player.round_number <= player.session.config['dividend_10_period']:
                player.dividend_clue = player.session.config['dividend_10_alt1']  
                player.dividend_clue_text=player.session.config['dividend_10_alt1_text']  

            if player.round_number <= player.session.config['dividend_9_period']:
                player.dividend_clue = player.session.config['dividend_9_alt1']  
                player.dividend_clue_text=player.session.config['dividend_9_alt1_text']  
            
            if player.round_number <= player.session.config['dividend_8_period']:
                player.dividend_clue = player.session.config['dividend_8_alt1']  
                player.dividend_clue_text=player.session.config['dividend_8_alt1_text']  

            if player.round_number <= player.session.config['dividend_7_period']:
                player.dividend_clue = player.session.config['dividend_7_alt1']  
                player.dividend_clue_text=player.session.config['dividend_7_alt1_text']              
            
            if player.round_number <= player.session.config['dividend_6_period']:
                player.dividend_clue = player.session.config['dividend_6_alt1']  
                player.dividend_clue_text=player.session.config['dividend_6_alt1_text']              
            
            if player.round_number <= player.session.config['dividend_5_period']:
                player.dividend_clue = player.session.config['dividend_5_alt1']  
                player.dividend_clue_text=player.session.config['dividend_5_alt1_text']  
            
            if player.round_number <= player.session.config['dividend_4_period']:
                player.dividend_clue = player.session.config['dividend_4_alt1']  
                player.dividend_clue_text=player.session.config['dividend_4_alt1_text']  
            
            if player.round_number <= player.session.config['dividend_3_period']:
                player.dividend_clue = player.session.config['dividend_3_alt1']  
                player.dividend_clue_text=player.session.config['dividend_3_alt1_text']  
           
            if player.round_number <= player.session.config['dividend_2_period']:
                player.dividend_clue = player.session.config['dividend_2_alt1']
                player.dividend_clue_text=player.session.config['dividend_2_alt1_text']  
           
            if player.round_number <= player.session.config['dividend_1_period']:
                player.dividend_clue = player.session.config['dividend_1_alt1'] 
                player.dividend_clue_text=player.session.config['dividend_1_alt1_text']  
        
        else:                                            # receive clue number two
            if player.round_number <= player.session.config['dividend_10_period']:
                player.dividend_clue = player.session.config['dividend_10_alt2']   
                player.dividend_clue_text = player.session.config['dividend_10_alt2_text']  

            if player.round_number <= player.session.config['dividend_9_period']:
                player.dividend_clue = player.session.config['dividend_9_alt2']   
                player.dividend_clue_text = player.session.config['dividend_9_alt2_text']  

            if player.round_number <= player.session.config['dividend_8_period']:
                player.dividend_clue = player.session.config['dividend_8_alt2']   
                player.dividend_clue_text = player.session.config['dividend_8_alt2_text']  

            if player.round_number <= player.session.config['dividend_7_period']:
                player.dividend_clue = player.session.config['dividend_7_alt2']   
                player.dividend_clue_text = player.session.config['dividend_7_alt2_text']  

            if player.round_number <= player.session.config['dividend_6_period']:
                player.dividend_clue = player.session.config['dividend_6_alt2']   
                player.dividend_clue_text = player.session.config['dividend_6_alt2_text']  

            if player.round_number <= player.session.config['dividend_5_period']:
                player.dividend_clue = player.session.config['dividend_5_alt2']   
                player.dividend_clue_text = player.session.config['dividend_5_alt2_text']  

            if player.round_number <= player.session.config['dividend_4_period']:
                player.dividend_clue = player.session.config['dividend_4_alt2']   
                player.dividend_clue_text = player.session.config['dividend_4_alt2_text']  
           
            if player.round_number <= player.session.config['dividend_3_period']:
                player.dividend_clue = player.session.config['dividend_3_alt2']   
                player.dividend_clue_text = player.session.config['dividend_3_alt2_text']                        
           
            if player.round_number <= player.session.config['dividend_2_period']:
                player.dividend_clue = player.session.config['dividend_2_alt2']
                player.dividend_clue_text = player.session.config['dividend_2_alt2_text']                
           
            if player.round_number <= player.session.config['dividend_1_period']:
                player.dividend_clue = player.session.config['dividend_1_alt2']                 
                player.dividend_clue_text = player.session.config['dividend_1_alt2_text'] 
    
    # dividend per sequence     

        if player.round_number <= player.session.config['dividend_10_period']:
            player.group.dividend = player.session.config['dividend_10']    
            player.group.world_state = player.session.config['state_10']     
    
        if player.round_number <= player.session.config['dividend_9_period']:
            player.group.dividend = player.session.config['dividend_9']    
            player.group.world_state = player.session.config['state_9']      
    
        if player.round_number <= player.session.config['dividend_8_period']:
            player.group.dividend = player.session.config['dividend_8']    
            player.group.world_state = player.session.config['state_8']  

        if player.round_number <= player.session.config['dividend_7_period']:
            player.group.dividend = player.session.config['dividend_7']    
            player.group.world_state = player.session.config['state_7']  

        if player.round_number <= player.session.config['dividend_6_period']:
            player.group.dividend = player.session.config['dividend_6']    
            player.group.world_state = player.session.config['state_6']  

        if player.round_number <= player.session.config['dividend_5_period']:
            player.group.dividend = player.session.config['dividend_5']    
            player.group.world_state = player.session.config['state_5']  

        if player.round_number <= player.session.config['dividend_4_period']:
            player.group.dividend = player.session.config['dividend_4']    
            player.group.world_state = player.session.config['state_4']  

        if player.round_number <= player.session.config['dividend_3_period']:
            player.group.dividend = player.session.config['dividend_3']    
            player.group.world_state = player.session.config['state_3']    

        if player.round_number <= player.session.config['dividend_2_period']:
            player.group.dividend = player.session.config['dividend_2']
            player.group.world_state = player.session.config['state_2']

        if player.round_number <= player.session.config['dividend_1_period']:
            player.group.dividend = player.session.config['dividend_1'] 
            player.group.world_state = player.session.config['state_1']

    # titles for rounds in sequences     
    if (group.round_number == group.session.config['dividend_1_period']) or (group.round_number ==group.session.config['dividend_2_period']) or \
    (group.round_number ==group.session.config['dividend_3_period']) or (group.round_number ==group.session.config['dividend_4_period']) or \
    (group.round_number ==group.session.config['dividend_5_period']) or (group.round_number ==group.session.config['dividend_6_period']) or \
    (group.round_number ==group.session.config['dividend_7_period']) or (group.round_number ==group.session.config['dividend_8_period']) or \
    (group.round_number ==group.session.config['dividend_9_period']) or (group.round_number ==group.session.config['dividend_10_period']):
        group.title = 'Third and Period Round of the Sequence'
        
    elif (group.round_number == group.session.config['dividend_1_period']-1) or (group.round_number == group.session.config['dividend_2_period']-1) or \
    (group.round_number == group.session.config['dividend_3_period']-1) or (group.round_number == group.session.config['dividend_4_period']-1) or \
    (group.round_number == group.session.config['dividend_5_period']-1) or (group.round_number == group.session.config['dividend_6_period']-1) or \
    (group.round_number == group.session.config['dividend_7_period']-1) or (group.round_number == group.session.config['dividend_8_period']-1) or \
    (group.round_number == group.session.config['dividend_9_period']-1) or (group.round_number == group.session.config['dividend_10_period']-1):
        group.title = 'Second Period of the Sequence'
    
    elif (group.round_number == group.session.config['dividend_1_period']-2) or (group.round_number == group.session.config['dividend_2_period']-2) or \
    (group.round_number == group.session.config['dividend_3_period']-2) or (group.round_number == group.session.config['dividend_4_period']-2) or \
    (group.round_number == group.session.config['dividend_5_period']-2) or (group.round_number == group.session.config['dividend_6_period']-2) or \
    (group.round_number == group.session.config['dividend_7_period']-2) or (group.round_number == group.session.config['dividend_8_period']-2) or \
    (group.round_number == group.session.config['dividend_9_period']-2) or (group.round_number == group.session.config['dividend_10_period']-2) :
...
class WaitToStart(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        if (group.round_number == 1) or (group.round_number == group.session.config['dividend_1_period']+1) or (group.round_number == group.session.config['dividend_2_period']+1) or \
        (group.round_number == group.session.config['dividend_3_period']+1) or (group.round_number == group.session.config['dividend_4_period']+1) or \
        (group.round_number == group.session.config['dividend_5_period']+1) or (group.round_number == group.session.config['dividend_6_period']+1) or \
        (group.round_number == group.session.config['dividend_7_period']+1) or (group.round_number == group.session.config['dividend_8_period']+1) or \
        (group.round_number == group.session.config['dividend_9_period']+1):
            new_sequence(group)
            assign_types(group)
        else:
...
class TradingOLG(Page):
    @staticmethod
    def is_displayed(player: Player):
        return  player.round_number <= player.session.config['num_rounds'] and player.is_playing is True
...
class Prediction(Page):
    ...
    def is_displayed(player: Player):
        return player.round_number <= player.session.config['num_rounds'] and \
               player.session.config['OLG'] is True and player.is_playing is False
...
class SequenceResults(Page):
  #  timeout_seconds = 30
    @staticmethod
    def is_displayed(player: Player):
        return ( (player.round_number == player.session.config['dividend_1_period']) or (player.round_number ==player.session.config['dividend_2_period']) or \
    (player.round_number == player.session.config['dividend_3_period']) or (player.round_number ==player.session.config['dividend_4_period']) or \
    (player.round_number == player.session.config['dividend_5_period']) or (player.round_number ==player.session.config['dividend_6_period']) or \
    (player.round_number == player.session.config['dividend_7_period']) or (player.round_number ==player.session.config['dividend_8_period']) or \
    (player.round_number == player.session.config['dividend_9_period']) or (player.round_number ==player.session.config['dividend_10_period'])) and player.round_number<=player.session.config['num_rounds']
