    ...
    if 'bid' in data:
        ...
        if is_ready:
            ...
            if game.bid1 is not None:
                if total_bids==0:
                   game.payoff1 = 1/3*player.session.config['V']
                else:
                    game.payoff1 = player.session.config['endowment']+float(game.bid1)/total_bids*player.session.config['V']-float(game.bid1)
                p1.last_bid=game.bid1
                p1.last_payoff=game.payoff1
                p1.last_prediction=game.prediction1
            if game.bid2 is not None:
                if total_bids==0:
                    game.payoff2 = 1/3*player.session.config['V']
                else:                
                    game.payoff2 = player.session.config['endowment']+float(game.bid2)/total_bids*player.session.config['V']-float(game.bid2)
                p2.last_bid=game.bid2
                p2.last_payoff=game.payoff2
                p2.last_prediction=game.prediction2
            if game.bid3 is not None:
                if total_bids==0:
                    game.payoff3 = 1/3*player.session.config['V']
                else:
                    game.payoff3 = player.session.config['endowment']+float(game.bid3)/total_bids*player.session.config['V']-float(game.bid3)
